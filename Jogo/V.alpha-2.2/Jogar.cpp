...
Jogar::Jogar()
{
    ...
    // Criar o objeto de texto para os itens
    itens_texto_obj = new QGraphicsTextItem;
    itens_texto_obj->setDefaultTextColor("Purple");
    itens_texto_obj->setFont(QFont("Arial",FONTE_TEXTO_ITEM,true));
    itens_texto_obj->setPos(LARGURA_TELA_X - ESPACO_TEXTO_ITEM,0);
    //itens_texto_obj->setPlainText("aaAAaaa");
    itens_texto_obj->setOpacity(1);

    //Colocar o objeto de texto do item na cena
    cena_de_jogo->addItem(itens_texto_obj);

    // o primeiro item_em_display é o display zerado
    (itens)[0] = new itens_em_display;
    //(*itens) = new itens_em_display;
    (itens)[0]->posicao_y = 0;
...
{
    int atual_quantidade_de_itens = (*itens)->quantidade;
    (itens)[0]->quantidade = (itens)[0]->quantidade + 1;
    qDebug()<<"Entrou em 'texto_item_adquirido(Itens item_adquirido): '"<< atual_quantidade_de_itens + 1;
    (itens)[atual_quantidade_de_itens + 1] = new itens_em_display;
    qDebug()<<"novo display: '"<< (itens)[atual_quantidade_de_itens + 1];
    /*(*(itens + atual_quantidade_de_itens))->posicao_y = atual_quantidade_de_itens;
    (*(itens + atual_quantidade_de_itens))->itens_texto_obj_strct = itens_texto_obj;
    cena_de_jogo->addItem((*(itens + atual_quantidade_de_itens))->itens_texto_obj_strct);
    ((*(itens + atual_quantidade_de_itens))->itens_texto_obj_strct)->setPlainText("aaAAaaa");
    (*(itens + atual_quantidade_de_itens))->tempo_texto_item_tela = new QTimer;
    (*(itens + atual_quantidade_de_itens))->tempo_texto_item_tela->start(1000);
    connect((*(itens + atual_quantidade_de_itens))->tempo_texto_item_tela, SIGNAL(timeout()),this,SLOT(retira_texto_item()));
    //placar_texto_obj->setPlainText("Pontuação: "+ QString::number(pontos));
    //connect(tempo);*/
}
...
{
    qDebug()<<"Entrou em 'texto_item_adquirido(Itens item_adquirido)'";
    //int atual_quantidade_de_itens = (*itens)->quantidade;
    //if(atual_quantidade_de_itens)
    {
        //(*itens)->quantidade = atual_quantidade_de_itens -1;
        //cena_de_jogo->removeItem((*(itens + atual_quantidade_de_itens))->itens_texto_obj_strct);
    }
}
